...
class FlameMorphableModel:
    ...
    def __init__(self, path, textureResolution = 256, trimPca = False, landmarksPathName = 'landmark_62_mp.txt', device='cuda'):
        ...
        self.shapeBasisSize = 299 # Flame can support up to 300 coefficients
        self.albedoBasisSize = 145
        self.expBasisSize = 100 # Flame can support up to 100 coefficients
        self.eyePoseBasisSize = 6 #Flame supports eye balls
        self.neckPoseBasisSize = 3 #Flame supports neck
        self.device = device
        # pathH5Model = path + '/model2017-1_face12_nomouth.h5' # refer to flame2023.pkl ?
        pathPKLModel = path + '/flame2023.pkl' # refer to flame2023.pkl ?
        pathAlbedoModel = path + '/albedoModel2020_face12_albedoPart.h5' #refer to flame model
        pathUV = path + '/uvParametrization.' + str(textureResolution) + '.pickle'
        pathLandmarks = path + '/' + landmarksPathName

        pathPickleFileName = path + '/flame2023_compact.pickle'
        ...
        if os.path.exists(pathPickleFileName) == False:
            ...
            if os.path.exists(pathPKLModel) == False:
                print('Download  "flame2023.pkl from flame website and put it inside ',path, ' and run again...', file=sys.stderr, flush=True)
                exit(0)
            with open(pathPKLModel, "rb") as f:
                self.flame_model = Struct(**pickle.load(f, encoding="latin1"))
            # We dont seem to have a shapeMean and all this info in a pickle file. Should look into it with DEBUG
            # print("loading shape basis...")
            self.faces = torch.Tensor(self.flame_model.f.astype(np.int64)).to(device).float()
            ...
            dict = {'diffuseAlbedoMean': self.diffuseAlbedoMean.cpu().numpy(),
            ...
            print("Loading flameModel from " + pathPickleFileName + "...")
